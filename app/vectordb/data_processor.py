...
import bisect
import ast
from app.vectordb.db import ChromaCollector
import app.vectordb.parameters as parameters
from app.vectordb.data_preprocessor import TextPreprocessorBuilder
...
def _clear_chunks(data_chunks, data_chunks_with_context, data_chunk_starting_indices):
    ...
    for chunk, context, index in zip(data_chunks, data_chunks_with_context, data_chunk_starting_indices):
        ...
        if seen_chunk_start:
            # If we've already seen this exact chunk, and the surrounding context very close to the seen chunk, then skip it.
...
def process_and_add_to_collector(corpus, clear_collector_before_adding=True):
    chunk_lens = [700]
    ...
    if clear_collector_before_adding:
        logging.info(f'Working on understanding the clear-mechanism')
        collector.clear()
    
    metadata = ast.literal_eval("{'source': 'direct-text'}")
